...
class X25519KeyExchange implements KeyExchange
{
    ...
    public function computeSharedSecret(KeyPair $ourKeyPair, string $peerPublicKey): string
    {
        if (!($ourKeyPair instanceof OpenSslKeyPair)) {
            throw new CryptoException('Invalid key pair type for X25519');
        }
        ...
        $sharedSecret = openssl_pkey_derive($peerKeyResource, $ourKeyPair->getPrivateKey(), 32);

        if ($sharedSecret === false) {
            throw new CryptoException('Failed to compute X25519 shared secret');
        }
